 ...
 * @param {TLanguage extends 'SolidityAST' ? import('@tevm/solc').SolcAst | import('@tevm/solc').SolcAst[] | import('solc-typed-ast').SourceUnit | import('solc-typed-ast').SourceUnit[] : string | string[]} source - The source code to validate
 * @param {import('../compile/CompileBaseOptions.js').CompileBaseOptions<TLanguage>} options - The compilation options
...
export const validateSolcVersion = (source, options, logger) => {
	...
	if (options.language === 'SolidityAST') {
		const astSources = /** @type {import('@tevm/solc').SolcAst[] | import('solc-typed-ast').SourceUnit[]} */ (
